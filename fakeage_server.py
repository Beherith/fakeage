...
class Game:
	...
	def addplayer(self,client,playername):
		...
		if playername in self.players.itervalues():
	...
	def getgamestate(self):
		...
		for playername in self.players.itervalues():
		...
		for player, score in score_sorted_player_list:  # what the fuck does this sort on?
	...
	def updatescoreorder (self):
		...
		for liername, lie in self.lies.iteritems():  # who chose which lie
			lieselectioncount = 0
			for selectorname, choice in self.choices.iteritems():
		...
		correctcount = 0
		for playername, choice in self.choices.iteritems():
			if choice == self.answer:
				#self.scores[playername] += 1
				correctcount += 1
	...
	def like_recieved(self,client,likes):
		...
		if player_who_liked in self.likes:
			print 'Player %s tried to like another submission (%s) despite already having chosen (%s)' % (
			player_who_liked, likes, self.likes[player_who_liked])
			return -1
...
if __name__ == "__main__":
	...
	httpserver = BaseHTTPServer.HTTPServer((my_ip, options.httpport), SimpleHTTPRequestHandler)
